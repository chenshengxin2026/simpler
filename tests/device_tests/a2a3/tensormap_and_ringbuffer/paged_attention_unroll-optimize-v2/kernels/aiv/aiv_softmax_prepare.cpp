...
static __aicore__ void softmax_prepare_n_impl(
    ...
    // ======== Pass 1: Find global row max (TRESHAPE, no GM round-trip) ========
    for (uint64_t i = 0; i < n_blocks; i++) {
        ...
        if (i == 0) {
            TRESHAPE(maxND_a, localMaxDN);
        } else {
            TRESHAPE(maxND_b, localMaxDN);
            TMAX(maxND_a, maxND_a, maxND_b);
        }
    }

    // Convert accumulated max back to DN for Pass 2's TROWEXPANDSUB
    TRESHAPE(globalMaxDN, maxND_a);

    // Store final global max to GM (needed by online_update)
    set_flag(PIPE_V, PIPE_MTE3, EVENT_ID0);
    wait_flag(PIPE_V, PIPE_MTE3, EVENT_ID0);
    TSTORE(mijGlobalDN, globalMaxDN);

    // ======== Pass 2: Compute softmax with global max (already in UB) ========
